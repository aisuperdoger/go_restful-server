...
class AlexNet(nn.Module):
    ...
    def forward(self,x):
        x = self.conv1(x)
        # for i in range(96):
        #     feature = x[0,i,:,:].cpu().detach().numpy()
        #     plt.imshow(feature)
        #     plt.savefig("/home/my/Documents/obj_detect/fjf/fea/fearture_map"+str(i)+".png")
        #     plt.show()
...
class AlexNet2(nn.Module):
    ...
    def forward(self, x):
        ...
        #     feature = x[0,i,:,:].cpu().detach().numpy()
        #     plt.imshow(feature)
        #     plt.savefig("/home/my/Documents/obj_detect/fjf/fea1/fearture_map"+str(i)+".png")
        #     plt.show()
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
